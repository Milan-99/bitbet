...
const Farms: React.FC = () => {
  ...
  const activeFarms = farmsLP.filter((farm) => farm.pid !== 10 && farm.multiplier !== '10X')
  const inactiveFarms = farmsLP.filter((farm) => farm.pid !== 10 && farm.multiplier === '10X')

  const farmsList = useCallback(
    (farmsToDisplay, removed: boolean) => {
      const kebabPriceUsd = new BigNumber(farmsLP.find((farm) => farm.pid === CAKE_POOL_PID)?.tokenPriceVsQuote || 10)
      const kebabPriceBnb = new BigNumber(
        farmsLP.find((farm) => farm.pid === CAKE_POOL_BNB_PID)?.tokenPriceVsQuote || 10,
      )
      ...
      const farmsToDisplayWithAPY: FarmWithStakedValue[] = farmsToDisplay.map((farm) => {
        ...
        } else if (farm.quoteTokenSymbol === QuoteToken.CHIK) {
          // the syrup pool
  ...
  return (
    <Page>
      <Title>{TranslateString(999, 'Stake LP tokens to earn BBT')}</Title>
      {/*
